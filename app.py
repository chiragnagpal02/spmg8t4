...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+mysqlconnector://sbrp_g2t8@localhost:3306/sbrp_g2t8'
...
class StaffReportingOfficer(db.Model):
    ...
    staff_id = db.Column(db.Integer, primary_key=True)
    RO_id = db.Column(db.Integer, db.ForeignKey('STAFF_DETAILS.staff_id'), primary_key=True)
...
class RoleListings(db.Model):
    ...
    role_listing_id = db.Column(db.Integer, primary_key=True, auto_increment=True)
    ...
    role_listing_source = db.Column(db.Integer, db.ForeignKey('STAFF_DETAILS.staff_id'), nullable=False)
...
class RoleApplications(db.Model):
    ...
    def json(self):
        return {"role_app_id": self.role_app_id, "role_listing_id": self.role_listing_id, "staff_id": self.staff_id, "role_app_status": self.role_app_status, "role_app_ts_create": self.role_app_ts_create}
...
@app.route("/create_role_listing/<int:role_listing_id>", methods=['POST']) #This is for HR to create a new role listing
...
@app.route("/view_role_applicant_skills/<int:staff_id>") #This is for HR to view the skills of a role applicant
def view_role_applicant_skills(staff_id):
    ...
    applicant_skills = db.session.query(SkillDetails.skill_name) \
...
@app.route("/apply_for_role/<int:role_app_id>", methods=['POST']) #This is for staff to apply for a role
def apply_for_role(role_app_id):
    ...
    existing_application = RoleApplications.query.filter_by(role_listing_id=role_listing_id, staff_id=staff_id).first()
...
@app.route("/withdraw_role_application/<int:role_app_id>/<int:staff_id>", methods=['PUT']) #This is for staff to withdraw their application for a role
...
# Check if existing application. If not, no application to withdraw from
    ...
    try:
        ...
        return jsonify(
            {
                "code": 200,
                "data": existing_application.json()
            }
        ), 200
    except Exception as e:
        logging.error(str(e))
