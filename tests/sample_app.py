...
sample_app.config["SQLALCHEMY_ENGINE_OPTIONS"] = {"pool_recycle": 299}

db = SQLAlchemy(sample_app)

CORS(sample_app)

sample_app.logger.setLevel(logging.INFO)
handler = RotatingFileHandler("app.log", maxBytes=10000, backupCount=1)
handler.setLevel(logging.INFO)
sample_app.logger.addHandler(handler)
...
@sample_app.route("/login/<string:username>/<string:password>")
...
@sample_app.route("/create_login", methods=["POST"])
def create_login():
    ...
    sys_role = data.get("sys_role")

    login_details = LoginDetails(staff_id, username, password, sys_role)
            ...
            jsonify(
                {
                    ...
                    "message": "An error occurred creating the login details.",
                }
            ),
            500,
        )

    return jsonify({"code": 201, "username": username, "password": password}), 201
...
@sample_app.route("/listingdetailsall")
...
@sample_app.route("/roledetailsall")
...
@sample_app.route("/listing/<int:role_listing_id>")
...
@sample_app.route("/get_role_listing/<int:role_listing_id>")
...
@sample_app.route("/details/<int:role_id>")
...
@sample_app.route("/openingsbydept")
...
@sample_app.route("/rolelistings_open")  # This is for staff, to see all open role listings
def get_all_open():
...
@sample_app.route("/create_role_listing/<int:role_id>/<int:staff_id>", methods=["POST"])  # This is for HR to create a new role listing
...
@sample_app.route("/update_role_listing/<int:role_listing_id>", methods=["PUT"])  # This is for HR to update an existing role listing
...
@sample_app.route("/get_all_managers")
...
@sample_app.route("/apply_for_role", methods=["POST"])  # This is for staff to apply for a role
def apply_for_role():
    data = request.get_json()
...
@sample_app.route("/get_required_skills/<int:role_id>", methods=["GET"])
...
@sample_app.route("/get_staff_skills/<int:staff_id>", methods=["GET"])
...
@sample_app.route("/get_application_status/<int:staff_id>/<int:role_listing_id>", methods=["GET"])
...
@sample_app.route("/get_applied_applications/<int:staff_id>", methods=["GET"])
...
if __name__ == "__main__":
    sample_app.run(host="0.0.0.0", port=5000, debug=True)
